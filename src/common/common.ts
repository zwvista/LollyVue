...
export function unitsFrom(info: string): SelectItem[] {
  const f = () => {
  ...
  return f().map((v, i) => new SelectItem(i + 1, v));
}

export function partsFrom(parts: string): SelectItem[] {
  return parts.split(',').map((v, i) => new SelectItem(i + 1, v));
}
